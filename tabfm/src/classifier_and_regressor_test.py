...
class OOFPredictionTest(absltest.TestCase):
  ...
  def test_regressor_predict_boosting(self):
    model = tabfm_model.TabFM(
        loss="rmse",
        max_classes=10,
        embed_dim=8,
    ...
    with mock.patch.object(
      ...
      self.assertTrue(mock_batch_forward.called)
      self.assertTrue(hasattr(regressor, "y_oof_scaled_"))
      self.assertEqual(regressor.y_oof_scaled_.shape, (2, 10))

      mock_batch_forward.reset_mock()
      test_preds = regressor.predict(X[:3])
      self.assertEqual(test_preds.shape, (3,))
      self.assertEqual(mock_batch_forward.call_count, 2)
...
class CalibrationTest(absltest.TestCase):
  ...
  def test_calibration_multiclass(self):
    ...
    for method in ["temperature", "vector", "dirichlet"]:
      classifier = TabFMClassifier(
          model=model,
          n_estimators=2,
...
class EnsemblePresetTest(absltest.TestCase):
  ...
  def test_regressor_ensemble_preset(self):
    ...
    self.assertEqual(reg.boosting_alpha, 0.1)
